...
Freeform live reconcile test — SHORT, HIGHUSDT — trailing pre-entry enabled.

Session starts with full config (5 levels, Reset TP on [3,4,5], SL, main TP, trailing).
Levels are placed ABOVE current price; fill as price rises.
...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------------
    print("\n=== PRE-CLEANUP ===")
    if service.get_session(symbol, "SHORT"):
    ...
    try:
        ...
        service._grid_tp_config[(symbol, "SHORT")] = [{"tp_percent": TP_PCT, "close_percent": 100}]
        service.enable_tpsl(symbol, "SHORT", sl_percent=SL_PCT, tp_percent=100.0, sl_mode="avg_entry")
        ...
        service.disable_tpsl(symbol, "SHORT")
        try:
            if service.get_session(symbol, "SHORT"):
                service.stop_session(symbol, "SHORT")
