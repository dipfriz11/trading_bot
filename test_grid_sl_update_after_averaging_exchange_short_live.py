...
if __name__ == "__main__":
    ...
    if leftover is not None:
        ...
        print("  stopped leftover SHORT session")
    else:
        print("  no leftover SHORT session")
    ...
    try:
        ...
        print(f"  entry={entry_price:.8f}  qty={short_qty}")
        print(f"  [TIMING] position opened in {time.time() - t0:.2f}s")

        if short_qty == 0 or entry_price is None:
            print("FAIL: SHORT position not opened")
            sys.exit(1)

        # ------------------------------------------------------------------
        # START SHORT GRID SESSION (averaging orders above current price)
        # ------------------------------------------------------------------
        ...
        # SHORT SL = entry * (1 + sl_percent/100)  → above entry
        # ------------------------------------------------------------------
        print("\n=== ENABLE TPSL (initial SL) ===")
        expected_initial_sl = entry_price * (1 + SL_PCT / 100)
        print(f"  entry_price={entry_price:.8f}")
        print(f"  expected initial SL triggerPrice={expected_initial_sl:.8f}  (entry * {1 + SL_PCT/100:.4f})")

        t2 = time.time()
        service._base_position_qty[(symbol, position_side)] = short_qty
        service.enable_tpsl(symbol, position_side, sl_percent=SL_PCT, tp_percent=TP_PCT)
        t_sl_placed = time.time()
        print(f"  [TIMING] initial SL placed in {t_sl_placed - t2:.2f}s")
        ...
        # Drag SHORT grid level to market (above current price) to trigger
        # ------------------------------------------------------------------
        print("\n=== WAIT FOR AVERAGING FILL ===")
        print("  Drag SHORT grid level to market to trigger averaging.")
        print("  (Grid orders are ABOVE current price for SHORT)")
        ...
        while time.time() - t_wait < TIMEOUT:
            ...
            if sess is not None and any(lvl.status == "filled" for lvl in sess.levels):
                ...
                print(f"  averaging detected after {t_fill_detected - t_wait:.1f}s")
                break
            time.sleep(2.0)
        ...
        lag = (t_after_sl_update - t_fill_detected) if t_fill_detected else None

        print(f"\n  new_entry={new_entry}")
        print(f"  new algoId={new_algo_id}")
        print(f"  new triggerPrice={new_trigger}")
        if expected_new_trigger:
            print(f"  expected triggerPrice={expected_new_trigger:.8f}  (new_entry * {1 + SL_PCT/100:.4f})")
        else:
            print("  expected triggerPrice=N/A")
        if lag is not None:
            print(f"  [TIMING] lag from fill detection to state read: {lag:.2f}s")
        ...
        if averaged:
            print("  PASS [1]: SHORT averaging fill detected")
        else:
        ...
        # [5] new triggerPrice matches SHORT formula
        ...
        # [6] triggerPrice shifted (entry changed after averaging)
        ...
        for pos in exchange.get_positions(symbol):
            if pos["positionSide"] == position_side:
                ...
                if qty > 0:
                    exchange.close_position(symbol, "buy", qty)
                    print(f"  closed SHORT position: qty={qty}")
                break
