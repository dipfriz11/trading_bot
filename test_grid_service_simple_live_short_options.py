...
if __name__ == "__main__":
    ...
    try:
        session = service.start_session(
            ...
            qty_mode="fixed",
            orders_count=5,
            first_price=0.00470943,
            last_price=0.00480315,
            distribution_mode="step",
            distribution_value=1.0,
        )

        print(f"\n=== SCENARIO 1: SHORT / step / fixed ===")
    ...
    try:
        ...
        print(f"\n=== SCENARIO 2: SHORT / density to end / fixed ===")
        print(f"session_id:     {session.session_id}")
        print(f"symbol:         {session.symbol}")
        print(f"position_side:  {session.position_side}")
        print(f"current market price: {exchange.get_price(symbol)}")
    ...
    # -------------------------------------------------------
    # 3) SHORT / density to start / fixed
    # -------------------------------------------------------
    try:
        session = service.start_session(
            symbol=symbol,
        ...
        print(f"\n=== SCENARIO 3: SHORT / density to start / fixed ===")
        print(f"session_id:     {session.session_id}")
        print(f"symbol:         {session.symbol}")
        print(f"position_side:  {session.position_side}")
        print(f"current market price: {exchange.get_price(symbol)}")
    ...
    try:
        ...
        print(f"\n=== SCENARIO 4: SHORT / step / multiplier ===")
        print(f"session_id:     {session.session_id}")
        print(f"symbol:         {session.symbol}")
        print(f"position_side:  {session.position_side}")
        print(f"current market price: {exchange.get_price(symbol)}")
