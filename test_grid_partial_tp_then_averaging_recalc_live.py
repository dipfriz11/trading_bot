...
if __name__ == "__main__":
    ...
    try:
        ...
        #
        # LONG:
        #   TP1 at +0.3%  — close, will fill naturally with small market move
        ...
        #
        # TP1 fills → _grid_tp_orders LONG goes 2 → 1
        # Averaging  → remaining TP2: new order_id, qty = full position, price repriced
        # ------------------------------------------------------------------
        print("\n=== PLACE TP ORDERS ===")
        ...
        tp1_initial_id    = initial_long_tps[0]["order_id"]
        tp2_initial_id    = initial_long_tps[1]["order_id"]

        print(f"\n  === INITIAL STATE ===")
        print(f"  LONG  TP ids:    {initial_long_ids}")
        print(f"  LONG  TP prices: {initial_long_prices}")
        print(f"    TP1 expected: entry*1.003 = {long_entry * 1.003:.8f}")
        print(f"    TP2 expected: entry*1.08  = {long_entry * 1.08:.8f}")
        ...
        # ------------------------------------------------------------------
        # PHASE 1: WAIT FOR LONG TP1 TO FILL NATURALLY
        # TP1 is at +0.3% from entry — small market move upward fills it.
        # watcher detects fill via check_tp_fills():
        #   _grid_tp_orders LONG: 2 → 1
        #   _base_position_qty: 14 → ~7  (position after partial close)
        # ------------------------------------------------------------------
        print(f"\n=== PHASE 1: WAITING FOR LONG TP1 FILL (natural) ===")
        print(f"  LONG TP1 price = {long_placed[0]['price']:.8f}  (+0.3% from entry={long_entry:.8f})")
        print(f"  Waiting up to 60s for market to reach TP1...")
        ...
        for _ in range(60):
            current_tps = service._grid_tp_orders.get((symbol, "LONG"), [])
        ...
        if not tp1_fill_detected:
            print("FAIL: LONG TP1 not filled within 60s — aborting")
            sys.exit(1)

        tp1_lag = tp1_fill_time - t_loop_start
        print(f"  [TIMING] LONG TP1 fill detected at +{tp1_lag:.1f}s from loop start")
        ...
        print(f"\n  === STATE AFTER TP1 FILL ===")
        print(f"  _grid_tp_orders LONG count: 2 -> 1")
        ...
        print(f"  LONG position qty after partial close: {pos_after_tp1_qty}")
        print(f"  _base_position_qty now:  {service._base_position_qty.get((symbol, 'LONG'))}")

        # Snapshot SHORT for isolation baseline
        short_snap_after_tp1   = list(service._grid_tp_orders.get((symbol, "SHORT"), []))
        short_ids_after_tp1    = {tp["order_id"] for tp in short_snap_after_tp1}
        short_qtys_after_tp1   = [tp["qty"]      for tp in short_snap_after_tp1]
        short_prices_after_tp1 = [f"{tp['price']:.8f}" for tp in short_snap_after_tp1]

        # ------------------------------------------------------------------
        # PHASE 2: MANUAL AVERAGING — drag LONG grid level[1] near market
        # After averaging fill watcher runs:
        #   check_grid_fills() → level[1] filled (position delta: ~7 + ~17 = ~24)
        #   update_grid_tp_orders_reprice():
        #     existing = [TP2]   (only remaining after TP1 fill)
        #     tp_qty   = full new position   (last/only entry → uses remainder)
        #     tp_price = new_avg_entry * (1 + 8.0/100)
        # Detection: remaining TP id changes (same count=1, new order_id)
        # ------------------------------------------------------------------
        print(f"\n=== PHASE 2: MANUAL ACTION — AVERAGE LONG POSITION ===")
        ...
        if not averaging_detected:
            print("FAIL: averaging fill not detected within 60s — aborting")
            sys.exit(1)

        averaging_lag = averaging_time - tp1_fill_time
        print(f"  [TIMING] averaging fill detected at +{averaging_lag:.1f}s after TP1 fill")
        ...
        # Expected TP2 price in reprice mode: new_avg_entry * (1 + 8.0/100)
        expected_tp2_price = (new_long_entry * (1 + remaining_tp2["tp_percent"] / 100)
                              if new_long_entry else None)

        print(f"\n  === STATE AFTER AVERAGING FILL ===")
        if new_long_entry:
            print(f"  new LONG avg entry:      {new_long_entry:.8f}")
        else:
            print(f"  new LONG avg entry: N/A")
        print(f"  new LONG position qty:   {new_long_qty}")
        ...
        if expected_tp2_price:
            print(f"  remaining TP expected price: {expected_tp2_price:.8f}"
                  f"  (formula: {new_long_entry:.8f} * (1 + {remaining_tp2['tp_percent']}/100))")
        print(f"  remaining TP qty:        {remaining_tp2['qty']}  (expected: full position ~{new_long_qty})")

        # Snapshot SHORT after averaging
        short_snap_after_avg   = list(service._grid_tp_orders.get((symbol, "SHORT"), []))
        short_ids_after_avg    = {tp["order_id"] for tp in short_snap_after_avg}
        short_qtys_after_avg   = [tp["qty"]      for tp in short_snap_after_avg]
        short_prices_after_avg = [f"{tp['price']:.8f}" for tp in short_snap_after_avg]
        ...
        print(f"  TP1 fill detected:      +{tp1_lag:.1f}s from loop start")
        print(f"  averaging detected:     +{averaging_lag:.1f}s after TP1 fill")
        ...
        # [7] TP1 LONG fill detected — count 2 → 1
        if tp1_fill_detected:
            print(f"  PASS: LONG TP1 fill detected  (count 2 -> 1 at +{tp1_lag:.1f}s)")
        else:
            print("  FAIL: LONG TP1 fill not detected within 60s")
            passed = False

        # [8] TP1 removed — remaining id matches original TP2, not TP1
        if after_tp1_remaining_id == tp2_initial_id:
            print(f"  PASS: TP1 removed from _grid_tp_orders  remaining=TP2 (id={after_tp1_remaining_id})")
        else:
            print(f"  FAIL: remaining id={after_tp1_remaining_id}  expected TP2 id={tp2_initial_id}")
            passed = False

        # [9] remaining TP is TP2 (tp_percent=8.0)
        if after_tp1_remaining_tp["tp_percent"] == 8.0:
            print(f"  PASS: remaining LONG TP is TP2  tp_percent={after_tp1_remaining_tp['tp_percent']}%")
        else:
        ...
        if averaging_detected:
            print(f"  PASS: averaging fill detected  (TP2 id changed at +{averaging_lag:.1f}s after TP1 fill)")
        else:
            print("  FAIL: averaging fill not detected within 60s")
            passed = False

        # [11] remaining TP2 qty = full new position (last/only TP gets remainder)
        actual_qty   = remaining_tp2["qty"]
        expected_qty = new_long_qty
        if abs(actual_qty - expected_qty) < 1.0:
            print(f"  PASS: remaining TP2 qty covers full position  qty={actual_qty}  position={expected_qty}")
        else:
            print(f"  FAIL: remaining TP2 qty={actual_qty}  expected ~{expected_qty}  (full new position)")
            passed = False

        # [12] remaining TP2 price repriced from new avg entry (reprice mode)
        ...
        if expected_tp2_price is not None:
            ...
            if price_ok:
                print(f"  PASS: remaining TP2 price correct (reprice mode)"
                      f"  actual={actual_price:.8f}  expected={expected_tp2_price:.8f}")
            else:
                print(f"  FAIL: remaining TP2 price={actual_price:.8f}"
                      f"  expected={expected_tp2_price:.8f}"
                      f"  (new_entry={new_long_entry:.8f} * 1.08)")
                passed = False
        else:
            print("  FAIL: could not read new LONG entry price to verify TP2 reprice")
            passed = False

        # [13] SHORT TP ids unchanged throughout
        if short_ids_after_avg == initial_short_ids:
            print(f"  PASS: SHORT TP ids unchanged throughout")
        else:
            print(f"  FAIL: SHORT TP ids changed  initial={initial_short_ids}  after={short_ids_after_avg}")
            passed = False

        # [14] SHORT TP qty unchanged throughout
        if short_qtys_after_avg == initial_short_qtys:
            print(f"  PASS: SHORT TP qty unchanged throughout")
        else:
            print(f"  FAIL: SHORT TP qty changed  initial={initial_short_qtys}  after={short_qtys_after_avg}")
            passed = False

        # [15] SHORT TP prices unchanged throughout
        if short_prices_after_avg == initial_short_prices:
            print(f"  PASS: SHORT TP prices unchanged throughout")
        else:
            print(f"  FAIL: SHORT TP prices changed  initial={initial_short_prices}  after={short_prices_after_avg}")
            passed = False
