...
if __name__ == "__main__":
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # MANUAL ACTION: averaging fill на LONG стороне
        # ------------------------------------------------------------------
        long_level1 = long_session.levels[0]
        print(f"\n=== MANUAL ACTION REQUIRED (LONG side only) ===")
        ...
        # ------------------------------------------------------------------
        # WAIT: LONG TP order_ids должны измениться
        # ------------------------------------------------------------------
        ...
        while time.time() < deadline:
            ...
            long_filled_count = sum(
                1 for lvl in long_session.levels if lvl.status == "filled"
            )
            if long_filled_count > 0 and t_fill_detected is None:
                t_fill_detected = time.time()
                print(f"  [TIMING] LONG fill detected at +{t_fill_detected - t_watcher_start:.1f}s")

            short_current_tp  = service._grid_tp_orders.get((symbol, "SHORT"), [])
            short_current_ids = {tp["order_id"] for tp in short_current_tp}
            print(
                f"  waiting...  LONG tp_ids={long_current_ids}  filled={long_filled_count}"
                f"  |  SHORT tp_ids={short_current_ids}"
            )
            time.sleep(2.0)
        ...
        if t_fill_detected:
            print(f"  LONG fill detected:   +{t_fill_detected - t_watcher_start:.1f}s from watcher start")
        if t_update_done_at:
            print(f"  LONG TP update done:  +{t_update_done_at - t_watcher_start:.1f}s from watcher start")
            if t_fill_detected:
                print(f"  update lag:           {t_update_done_at - t_fill_detected:.2f}s")
        ...
        # [1] LONG и SHORT позиции открыты
        if long_qty > 0:
        ...
        # [2] LONG и SHORT sessions существуют
        ...
        # [3] initial TP orders were placed
        ...
        # [4] averaging fill detected only on LONG
        if update_done:
            print("  PASS: averaging fill detected on LONG side")
        else:
            print("  FAIL: LONG averaging fill not detected within 30s")
            passed = False

        long_updated_tp  = service._grid_tp_orders.get((symbol, "LONG"),  [])
        short_updated_tp = service._grid_tp_orders.get((symbol, "SHORT"), [])

        long_updated_ids    = {tp["order_id"] for tp in long_updated_tp}
        long_updated_prices = [tp["price"] for tp in long_updated_tp]
        long_updated_qtys   = [tp["qty"]   for tp in long_updated_tp]

        short_updated_ids    = {tp["order_id"] for tp in short_updated_tp}
        short_updated_prices = [tp["price"] for tp in short_updated_tp]
        short_updated_qtys   = [tp["qty"]   for tp in short_updated_tp]

        # [5] LONG TP order_ids changed
        if long_updated_ids != long_initial_ids:
            print(f"  PASS: LONG TP order_ids changed  old={long_initial_ids}  new={long_updated_ids}")
        else:
            print(f"  FAIL: LONG TP order_ids did not change")
            passed = False

        # [6] LONG TP qty increased
        sum_long_initial = sum(long_initial_qtys)
        sum_long_updated = sum(long_updated_qtys)
        if sum_long_updated > sum_long_initial:
            print(f"  PASS: LONG TP qty increased  {sum_long_initial} -> {sum_long_updated}")
            for i, tp in enumerate(long_updated_tp):
                print(f"    TP[{i}] qty={tp['qty']}  (was {long_initial_qtys[i]})")
        else:
            print(f"  FAIL: LONG TP qty not increased  {sum_long_initial} -> {sum_long_updated}")
            passed = False

        # [7] LONG TP prices unchanged (fixed mode — default)
        prices_unchanged = (
            len(long_updated_tp) == len(long_initial_prices)
            and all(
                abs(long_updated_tp[i]["price"] - long_initial_prices[i]) < 1e-9
                for i in range(len(long_updated_tp))
            )
        )
        if prices_unchanged:
            print(f"  PASS: LONG TP prices unchanged (fixed mode)")
            for i, tp in enumerate(long_updated_tp):
                print(f"    TP[{i}] price={tp['price']:.8f}  (was {long_initial_prices[i]:.8f})")
        else:
            print(f"  FAIL: LONG TP prices changed unexpectedly")
            passed = False

        # [8] SHORT TP order_ids НЕ changed
        if short_updated_ids == short_initial_ids:
            print(f"  PASS: SHORT TP order_ids unchanged  {short_updated_ids}")
        else:
            print(f"  FAIL: SHORT TP order_ids changed unexpectedly  old={short_initial_ids}  new={short_updated_ids}")
            passed = False

        # [9] SHORT TP qty НЕ changed
        if short_updated_qtys == short_initial_qtys:
            print(f"  PASS: SHORT TP qty unchanged  {short_updated_qtys}")
        else:
            print(f"  FAIL: SHORT TP qty changed  {short_initial_qtys} -> {short_updated_qtys}")
            passed = False

        # [10] SHORT TP prices НЕ changed
        prices_short_ok = (
            len(short_updated_tp) == len(short_initial_prices)
            and all(
                abs(short_updated_tp[i]["price"] - short_initial_prices[i]) < 1e-9
                for i in range(len(short_updated_tp))
            )
        )
        if prices_short_ok:
            print(f"  PASS: SHORT TP prices unchanged  {[f'{p:.8f}' for p in short_updated_prices]}")
        else:
            print(f"  FAIL: SHORT TP prices changed unexpectedly")
            passed = False

        # [11] обе sessions still alive
        ...
        # [12] watcher still watching both sides
