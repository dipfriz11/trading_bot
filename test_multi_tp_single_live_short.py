# test_multi_tp_single_live_short.py
#
# Live test: SingleOrderStrategy market SHORT entry, multi-TP + single SL (v1).
# Зеркало test_multi_tp_single_live.py для SHORT стороны.
#
...
#   2. order_manager.tpsl["SHORT"] содержит tps (2 шт.) + sl
...
if __name__ == "__main__":
    ...
    coin_cfg = {
        ...
        "short": {
            "usdt_amount": 8.5,
            "sl_percent":  10.0,   # +10% от entry — точно не триггернётся
            "take_profits": [
    ...
    widget_config = {
        "id":       f"test_multi_tp_short_{symbol.lower()}",
        "symbol":   symbol,
        "exchange": "binance",
        "market":   "futures",
    ...
    try:
        # ------------------------------------------------------------------
        # STEP 1: execute("sell") → market entry + multi-TP/SL
        # ------------------------------------------------------------------
