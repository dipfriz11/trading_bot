...
Freeform live reconcile test — HEDGE mode, HIGHUSDT — LONG + SHORT simultaneously.

Both legs run independently on the same symbol in Binance hedge mode.
Session starts with full config (5 levels, Reset TP on [3,4,5], SL, main TP, trailing).
After start: no scripted phases. User can freely interact on exchange.

System reconciles automatically via GridTrailingWatcher (one watcher, two watch slots).
Test prints state snapshots on every change + 30s heartbeat.
Exits when: both LONG and SHORT legs are done (position=0 + no active orders).
"""
import sys
import io
import time
...
def _get_pos(exchange, symbol, position_side):
...
def _leg_snap(service, exchange, session, symbol, position_side):
    pos_qty, entry = _get_pos(exchange, symbol, position_side)
...
def _snap_key(service, exchange, long_session, short_session, symbol):
    return {
        "long":  _leg_snap(service, exchange, long_session,  symbol, "LONG"),
        "short": _leg_snap(service, exchange, short_session, symbol, "SHORT"),
    }


def _print_leg(service, exchange, session, symbol, position_side, elapsed):
    pos_qty, entry = _get_pos(exchange, symbol, position_side)
    key = (symbol, position_side)
    ...
    print(f"  {position_side}: pos={pos_qty}  entry={entry:.8f}")
...
def _print_snap(service, exchange, long_session, short_session, symbol, elapsed, tag="SNAP"):
    mins = int(elapsed) // 60
    secs = int(elapsed) % 60
    ts   = f"{mins:02d}:{secs:02d}"
    print(f"\n[{tag}] {ts}")
    _print_leg(service, exchange, long_session,  symbol, "LONG",  elapsed)
    _print_leg(service, exchange, short_session, symbol, "SHORT", elapsed)


def _leg_done(service, exchange, session, symbol, position_side):
    pos_qty, _ = _get_pos(exchange, symbol, position_side)
    if pos_qty > 0:
        return False
    key = (symbol, position_side)
...
def _is_cycle_done(service, exchange, long_session, short_session, symbol):
    return (_leg_done(service, exchange, long_session,  symbol, "LONG") and
            _leg_done(service, exchange, short_session, symbol, "SHORT"))


def _start_leg(service, exchange, symbol, position_side):
    ...
    print(f"  {position_side} session_id={session.session_id}")
    for lvl in session.levels:
        print(f"    [{lvl.index}] order_id={lvl.order_id}  price={lvl.price:.8f}  qty={lvl.qty}  use_reset_tp={lvl.use_reset_tp}")

    service.set_tp_update_mode(symbol, position_side, "reprice")
    service._grid_tp_config[(symbol, position_side)] = [{"tp_percent": TP_PCT, "close_percent": 100}]
    service.enable_tpsl(symbol, position_side, sl_percent=SL_PCT, tp_percent=100.0, sl_mode="avg_entry")
    ...
    print(f"  {position_side} trailing enabled  anchor={anchor:.8f}  step={TRAILING_STEP_PCT}%")
    return session
...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------------
    print("\n=== PRE-CLEANUP ===")
    for side, close_side in (("LONG", "sell"), ("SHORT", "buy")):
        if service.get_session(symbol, side):
        ...
        for pos in exchange.get_positions(symbol):
            if pos["positionSide"] == side:
                ...
                if qty > 0:
                    ...
                    print(f"  closed leftover {side} position  qty={qty}")
    time.sleep(1.0)
    ...
    try:
        ...
        # ------------------------------------------------------------------
        print("\n=== START SESSIONS ===")
        long_session  = _start_leg(service, exchange, symbol, "LONG")
        short_session = _start_leg(service, exchange, symbol, "SHORT")

        watcher.start_watching(symbol, "LONG")
        watcher.start_watching(symbol, "SHORT")
        print(f"\n  watcher started for both legs  SL_PCT={SL_PCT}%  TP_PCT={TP_PCT}%  RESET_CLOSE={RESET_CLOSE_PCT}%  TRAILING_STEP={TRAILING_STEP_PCT}%")
        print(f"\n  *** Free to act on exchange. Test exits when both legs: position=0 + no active orders. ***\n")
        ...
        while True:
            ...
            cur_state = _snap_key(service, exchange, long_session, short_session, symbol)

            changed   = cur_state != prev_state
            heartbeat = (now - last_hb_time) >= HEARTBEAT_EVERY

            if changed:
                _print_snap(service, exchange, long_session, short_session, symbol, elapsed, tag="CHANGE")
                prev_state   = cur_state
                last_hb_time = now
            elif heartbeat:
                _print_snap(service, exchange, long_session, short_session, symbol, elapsed, tag="HB")
                last_hb_time = now

            if _is_cycle_done(service, exchange, long_session, short_session, symbol):
                print(f"\n[DONE] both legs complete  elapsed={elapsed:.0f}s")
                break
        ...
        for side, close_side in (("LONG", "sell"), ("SHORT", "buy")):
            ...
            if rtp:
                try:
                    ...
                    print(f"  cancelled {side} Reset TP  order_id={rtp['order_id']}")
                except Exception as ex:
                    print(f"  cancel {side} Reset TP error (ignored): {ex}")
            for tp in service._grid_tp_orders.pop(key, []):
                try:
                    exchange.cancel_order(symbol, tp["order_id"])
                    print(f"  cancelled {side} main TP  order_id={tp['order_id']}")
                except Exception:
                    pass
            service._tp_update_mode.pop(key, None)
            service.disable_tpsl(symbol, side)
            try:
                if service.get_session(symbol, side):
                    service.stop_session(symbol, side)
                    print(f"  stopped {side} session")
            except Exception as ex:
                print(f"  stop_session {side} error (ignored): {ex}")
            for pos in exchange.get_positions(symbol):
                if pos["positionSide"] == side:
                    qty = abs(float(pos["positionAmt"]))
                    if qty > 0:
